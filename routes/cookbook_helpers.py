...
def _check_llama_cpp_conditional_fallback(rest: str) -> bool:
    """Accept the GGUF native-or-python fallback shape generated by the UI."""
    prefix = "if command -v llama-server >/dev/null 2>&1; then "
    mid = "; else "
    suffix = "; fi"
    if not rest.startswith(prefix) or not rest.endswith(suffix) or mid not in rest:
        return False
    body = rest[len(prefix):-len(suffix)]
    native, fallback = body.split(mid, 1)
    _check_serve_binary(native.strip())
    _check_serve_binary(fallback.strip())
    return True
...
def _validate_serve_cmd(v: str | None) -> str | None:
    ...
    if m:
        rest = v[m.end():]
        if _check_llama_cpp_conditional_fallback(rest.strip()):
            return v
