...
def _ping_endpoint(base_url: str, api_key: str = None, timeout: float = 1.5) -> Dict[str, Any]:
    ...
    try:
        ...
        if root and root != base:
            # vLLM/SGLang/llama.cpp-style OpenAI-compatible servers often expose
            # health/version at the server root while GET /v1 correctly returns
            # 404 because /v1 is only an API namespace.
            for path in ("/health", "/version", "/ping"):
                try:
                    r = httpx.get(root + path, headers=headers, timeout=timeout)
                    result = _result_from_response(r)
                    if result["reachable"]:
                        return result
                    last_error = result.get("error")
